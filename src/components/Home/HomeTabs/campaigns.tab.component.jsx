...
function CampaignsTabComponent () {
	...
	const {occurrences} = useContext(GoalsContext);
		...
		<div className="row">
			<div className="col-12">
				...
				{campaigns.map(campaign => {
					const progress = getCampaignProgress(campaign, occurrences);
					...
					return <article key={campaign.id} className="dark-primary-bg border border-secondary rounded p-3 mb-3">
						...
						<p className="small inactive-fg mb-0 mt-2">{progress.completedOccurrences} of {progress.totalOccurrences} occurrences completed</p>
					</article>;
				})}
			</div>
		</div>
